...
col_wrap_word: 		.space 65	# col_wrap_word

dg_wrap_word: 		.space 65 	# dg_wrap_word

row_wrap_word: 		.space 65	# row_wrap_word
...
PRINT_LOOP_HOR:
	lb $t9, 0($a0) 			# $t9 = *word
	beq $t9, $t0, PRINT_END_HOR	# branch if *word == '\n'
	beq $t9, $0, PRINT_END_HOR	# branch if *word == '\0'
	move $s5, $a0			# $s0 = word
	move $a0, $t9			# $a0 = *word
...
GET_ROW_WORD:
	bge $s4, $t2, LOOP_COL_HOR	# while (y_coord < total_cols)
	addi $t5, $t2, 1		# $t5 = total_cols + 1
	mul $t6, $s3, $t5		# $t6 = x_coord * (total_cols + 1) for '\n')
	add $t6, $t6, $s4		# grid_idx = y_coord + x_coord * total_cols
	lb $t7, grid($t6)		# t7 = grid[grid_idx]
	sb $t7, row_wrap_word($s4)	# row_wrap_word[y_coord] = grid[grid_idx]
	add $t8, $s4, $t2		# t8 = y_coord + total_cols
	sb $t7, row_wrap_word($t8)	# row_wrap_word[y_coord + total_cols] = grid[grid_idx]
	addi $s4, $s4, 1		# y_coord++
	j GET_ROW_WORD
	
LOOP_COL_HOR:
	bge $t4, $t2, NEXT_ROW_HOR	# while col_start_idx < total_cols
	li $t3, 0			# idx = 0
	
LOOP_DIC_HOR:
	bge $t3, $s7, DIC_END_HOR	# for (idx < dict_num_words)
	li $t0, 4			# int offset = 4
	mul $t5, $t0, $t3		# $t5 = idx * offset
	lw $t6, dictionary_idx($t5)	# $t6 = dictionary_idx[idx]
	la $t5, dictionary($t6)		# char *word = dictionary + dictionary_idx[idx]
	la $t7, row_wrap_word($t4)	# $t7 = &row_wrap_word[col_start_idx)
	move $a0, $t7			# $a0 = &row_wrap_word[col_start_idx)
	move $a1, $t5			# $a1 = word
	jal contain_hor
	
contain_returns_hor:
	beqz $v0, NEXT_WORD_HOR		# if (contain(&row_wrap_word[col_start_idx], word))
	li $s2, 1			# contains = 1
...
LOOP_ROW_VER:
	...
	sb $t3, col_wrap_word($s3)	# col_wrap_word[x_coord] = grid[grid_idx]
	add $t0, $s3, $t1		# $t0 = x_coord + total_rows
	sb $t3, col_wrap_word($t0)	# col_wrap_word[x_coord + total_rows] = grid[grid_idx]
	addi $s3, $s3, 1		# x_coord++
	j LOOP_ROW_VER

NEXT_COL_VER:	
	li $s1, 0			# row_start_idx = 0
	
COL_WORD_VER:
	bge $s1, $t1, COL_WORD_END	# while (row_start_idx < total_rows)
	li $t3, 0			# idx = 0
...
LOOP_DIC_VER:	
	...
	la $s0, col_wrap_word($s1)		# $s0 = &col_word[start_idx]
...
SW_DG:
	...
	lb $t7, grid($t6)		# $t7 = grid[grid_idx]
	sb $t7, dg_wrap_word($t0)		# dg_word[dg_idx] = grid[grid_idx]
	addi $t0, $t0, 1		# dg_idx++
	addi $s3, $s3, 1		# x_coord++
	addi $s4, $s4, 1		# y_coord++
	j SW_DG
...
LOOP_DIC_DG:
	...
	la $t9, dg_wrap_word			# $t9 = &dg_word[0]
